...
function probando(req, res){
    res.status(200).send({
        message: "hola desde el controlador de publicacionnes"
    });
}
...
function savePublication(req, res){
    ...
    publication.save((err, publicationStored) => {
        ...
        if(!publicationStored) return res.status(404).send({message: 'La publicacion no ha podido ser guardada'});
...
function getPublications(req, res){
    ...
    Follow.find({user: req.user.sub}).populate('followed').exec((err, follows) => {
        ...
        follows_clean.push(req.user.sub); //para agregar nuestras publicaciones al feed
...
//nuevo metodo para llamar las publicaciones en el perfil del usuario
...
function uploadImage(req, res){         //subir archivos de imagen //avatar de usuario
    var publicationId = req.params.id;
